...
(check-equal? (run "(lam types (\"int\", \"float\";) x := x + 2.0) : 5.0")
7.0)
(check-equal? (run "(lam types (\"int\", str;) x := x + 2.0) : 5.0")
7.0)
(check-equal? (run "(lam types lst x := x + 2.0) : 5.0")
7.0)
(check-equal? (run "(lam types lst x, y := + x + float y + 2.0) : 5.0 : 8")
15.0)
(check-equal? (run "(lam types lst x, types (\"int\", \"string\";) y := + x + float y + 2.0) : 5.0 : 8")
15.0)
(check-exn exn:fail? (lambda ()
  (run "(lam types lst x, types (\"int\", \"string\";) y := + x + float y + 2.0) : 5.0 : 8.0")))
